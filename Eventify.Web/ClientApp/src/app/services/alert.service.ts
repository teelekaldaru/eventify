...
import { MessageType } from '../models/system/request-result';
...
export class AlertService {
    ...
    showNotImplementedMessage() {
        this.showMessage('Not implemented yet', null, MessageSeverity.warn);
    }

    showMessage(summary: string);
    showMessage(summary: string, detail: string, severity: MessageSeverity);
    showMessage(
        summaryAndDetails: string[],
        summaryAndDetailsSeparator: string,
        severity: MessageSeverity
    );
    showMessage(
        response: HttpResponseBase,
        ignoreValueUseNull: string,
        severity: MessageSeverity
    );
    showMessage(
        data: any,
        ...
        if (data instanceof Array) {
            for (const message of data) {
                const msgObject = Utilities.splitInTwo(
                    message,
                    separatorOrDetail
                );

                this.showMessageHelper(
                    msgObject.firstPart,
                    msgObject.secondPart,
                    severity,
                    false
                );
            }
    ...
    showResponseMessages(messages: SimpleMessage[], showWarnings = true) {
    ...
    showResponseMessagesAsSticky(messages: SimpleMessage[]) {
        messages.forEach((message) => {
            this.showStickyMessage(
                message.header,
                message.description,
    ...
    showStickyMessage(
        ...
        if (data instanceof Array) {
            ...
            if (error) {
                ...
                }", Summary: "${data}", Detail: "${separatorOrDetail}", Error: "${Utilities.safeStringify(
                    error
                )}"`;
